...
let productsElectronics: Product[]  = [
  {
    id: 1,
...
app.get(
  ...
  (req: Request, res: Response) => {
    ...
    const product = productsElectronics.find((p) => p.id === id);

    if (!product) {
      errorResponse(res, "Produk tidak ditemukan!");
    }
    if (!productsElectronics) {
...
app.post(
  ...
  (req: Request, res: Response) => {
    ...
    successResponse(res, "Produk berhasil ditambahkan!", productsElectronics, null, 201);
  }
);
